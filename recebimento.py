...
def _load_recebimento():
    ...
    SELECT
        ...
        RAZAO_SOCIAL,
        FILIAL,
        USUARIO_CONFERENCIA_2,
        QUANTIDADE_CONFERIDA,
        QUANTIDADE_FALTANTE,
        QUANTIDADE_TOTAL,
        PEDIDO
    FROM db_visual_SALOG.dbo.VW_PAINEL_STATUS_RECEBIMENTO_PEDIDO_WMS
    """
...
def show():
    ...
    df["Conferente"] = (
        df["USUARIO_CONFERENCIA_2"]
        .fillna("Sem Conferente")
        .str.split(".")
        .str[0]
        .str.title()
    )
    ...
    <div style="display:flex; align-items:center; gap:16px; flex-wrap:wrap; margin-bottom:4px;">
        <h1 style="margin:0;">PAINEL DE RECEBIMENTO - SALOG</h1>
        <div style="
    ...
    <p style='color:#FFFFFF;margin-top:4px;margin-bottom:0;'>Monitoramento de Recebimentos</p>
    ...
    # FILTROS
    # =========================
    f1, f2, f3, f4 = st.columns([0.6, 0.6, 1.2, 1.2])

    with f1:
        data_inicio = st.date_input(
            "Data Inicial",
            df["DATA_CONSOLIDACAO"].min(),
            format="DD/MM/YYYY"
        )
    with f2:
        data_fim = st.date_input(
            "Data Final",
            df["DATA_CONSOLIDACAO"].max(),
            format="DD/MM/YYYY"
        )
    with f3:
    ...
    # =========================
    df = df[
        (df["DATA_CONSOLIDACAO"] >= pd.to_datetime(data_inicio))
        & (df["DATA_CONSOLIDACAO"] <= pd.to_datetime(data_fim))
    ]
    if filiais:
    ...
    # =========================
    recebimentos      = df["PEDIDO"].nunique()
    finalizados       = df[df["STATUS"] == "FINALIZADO"]["PEDIDO"].nunique()
    aguardando_end    = df[df["STATUS"] == "AGUARANDO ENDEREÇAMENTO"]["PEDIDO"].nunique()
    aguardando_conf   = df[df["STATUS"] == "AGUARDANDO CONFERÊNCIA"]["PEDIDO"].nunique()
    em_conferencia    = df[df["STATUS"] == "EM CONFERÊNCIA"]["PEDIDO"].nunique()
    aguardando_consol = df[df["STATUS"] == "AGUARDANDO CONSOLIDAÇÃO"]["PEDIDO"].nunique()

    pct_fin   = int((finalizados / recebimentos * 100) if recebimentos else 0)
    pct_end   = int((aguardando_end / recebimentos * 100) if recebimentos else 0)
    pct_conf  = int((aguardando_conf / recebimentos * 100) if recebimentos else 0)
    pct_em    = int((em_conferencia / recebimentos * 100) if recebimentos else 0)
    pct_consol= int((aguardando_consol / recebimentos * 100) if recebimentos else 0)

    # badge: verde se 0, âmbar se >0 e baixo, vermelho se alto
    ...
    c_end_color, c_end_bg, c_end_border       = badge(aguardando_end, inverso=True)
    c_conf_color, c_conf_bg, c_conf_border    = badge(aguardando_conf, inverso=True)
    c_em_color, c_em_bg, c_em_border          = badge(em_conferencia, inverso=True)
    c_consol_color, c_consol_bg, c_consol_border = badge(aguardando_consol, inverso=True)

    k1, k2, k3, k4, k5, k6 = st.columns(6)
    with k1:
        st.markdown(kpi_card("Recebimentos", recebimentos, 100, "#22D3EE",
    ...
    with k3:
        st.markdown(kpi_card("AG. Conferência", aguardando_conf, pct_conf, c_conf_color,
                             c_conf_color, c_conf_bg, c_conf_border), unsafe_allow_html=True)
    with k4:
        st.markdown(kpi_card("Em Conferência", em_conferencia, pct_em, c_em_color,
                             c_em_color, c_em_bg, c_em_border), unsafe_allow_html=True)
    with k5:
        st.markdown(kpi_card("AG. Endereçamento", aguardando_end, pct_end, c_end_color,
                             c_end_color, c_end_bg, c_end_border), unsafe_allow_html=True)
    with k6:
        st.markdown(kpi_card("AG. Consolidação", aguardando_consol, pct_consol, c_consol_color,
                             c_consol_color, c_consol_bg, c_consol_border), unsafe_allow_html=True)
    ...
    with c1:
        st.subheader("Status Recebimentos")
        status_df = df.groupby("STATUS")["PEDIDO"].count().reset_index()
        fig1 = go.Figure(go.Pie(
            labels=status_df["STATUS"], values=status_df["PEDIDO"],
            hole=0.42,
        ...
        layout_fig1["annotations"] = [dict(
            text=f"<b>{recebimentos}</b><br>Pedidos",
            x=0.5, y=0.5,
    ...
    with c2:
        st.subheader("Quantidade Conferida")
        conf_df = df[["QUANTIDADE_CONFERIDA", "QUANTIDADE_FALTANTE"]].sum().reset_index()
        conf_df.columns = ["Tipo", "Quantidade"]
        fig2 = go.Figure(go.Pie(
            labels=conf_df["Tipo"], values=conf_df["Quantidade"],
            hole=0.68,
        ...
        layout_fig2["annotations"] = [dict(
            text=f"<b>{int(conf_df['Quantidade'].sum()):,}</b>",
            x=0.5, y=0.5,
    ...
    with c3:
        ...
        conf_rank = (
            df.groupby("Conferente")["PEDIDO"].count().reset_index()
            .sort_values("PEDIDO", ascending=True).tail(10)
        )
        fig3 = px.bar(
            conf_rank, x="PEDIDO", y="Conferente", orientation="h",
            color="PEDIDO", color_continuous_scale=[[0, "#0D3320"], [1, GREEN]], text="PEDIDO"
        )
    ...
    with c4:
        ...
        dia_df = (
            df.groupby("DATA_CONSOLIDACAO")["PEDIDO"].count().reset_index()
            .rename(columns={"PEDIDO": "Pedidos"}).sort_values("DATA_CONSOLIDACAO")
        )
        dia_df["Media_Movel"] = dia_df["Pedidos"].rolling(7, min_periods=1).mean()

        fig4 = go.Figure()
        fig4.add_trace(go.Scatter(
            x=dia_df["DATA_CONSOLIDACAO"], y=dia_df["Pedidos"],
        ...
        fig4.add_trace(go.Scatter(
            x=dia_df["DATA_CONSOLIDACAO"], y=dia_df["Media_Movel"],
    ...
    cliente_rank = (
        df.groupby("RAZAO_SOCIAL")["PEDIDO"].count().reset_index()
        .rename(columns={"RAZAO_SOCIAL": "Cliente", "PEDIDO": "Pedidos"})
    ...
    fig_cliente.update_traces(textposition="outside", textfont_color="#C8D6E5", marker_line_width=0)
    fig_cliente.update_layout(
        **base_layout(height=450),
        coloraxis_showscale=False, bargap=0.3,
        xaxis_title="Pedidos", yaxis_title=""
    )
    fig_cliente.update_layout(margin=dict(l=320, r=60, t=36, b=16))
    ...
    df_display = df[[
        "DATA_CONSOLIDACAO", "STATUS", "RAZAO_SOCIAL",
        "FILIAL", "Conferente", "QUANTIDADE_CONFERIDA"
    ]].copy()
    df_display["DATA_CONSOLIDACAO"] = df_display["DATA_CONSOLIDACAO"].dt.strftime("%Y-%m-%d %H:%M")
    df_display = df_display.sort_values("DATA_CONSOLIDACAO", ascending=False)
    ...
    rows_json = df_display.rename(columns={
        ...
        "FILIAL":               "filial",
        "Conferente":           "user",
        "QUANTIDADE_CONFERIDA": "qty"
    }).to_dict(orient="records")

    max_qty = float(df_display["QUANTIDADE_CONFERIDA"].max() or 1)
...
<style>
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  .s-conf {{ background:rgba(34,211,238,.1); color:#22D3EE; border:1px solid rgba(34,211,238,.2); }}
  ...
  .razao {{ max-width:200px; overflow:hidden; text-overflow:ellipsis; color:#8A9AB0; }}
...
<div class="panel">
  <div class="panel-header">
    <span class="panel-title">Base Operacional</span>
    ...
    <table>
      <thead>
        <tr>
          ...
          <th>Filial</th>
          <th>Conferente</th>
          <th style="text-align:right">Qtd. Conf.</th>
        </tr>
      </thead>
  ...
  <div class="footer-bar">
    <div>Total conferido: <span id="totalConf">0</span> unid.</div>
...
function statusClass(s) {{
  ...
  if (s.includes('CONFERENCIA') || s.includes('CONFERÊNCIA'))     return 's-conf';
  return 's-agu';
}}
...
function updateStats() {{
  ...
  document.getElementById('totalConf').textContent = total.toLocaleString('pt-BR');
